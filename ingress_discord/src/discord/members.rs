...
lazy_static! {
    static ref CLIENT: Client = Client::new(std::env::var("DISCORD_TOKEN").expect("DISCORD_TOKEN not set"));
    static ref API_URL: String = std::env::var("DATASYNC_API_URL").expect("DATASYNC_API_URL not set");
}
...
pub async fn update_users(pool: Pool<ConnectionManager<SqliteConnection>>, filter: Option<GuildRoleFilter>) -> anyhow::Result<()> {
    log::info!("Updating users...");

    let name = CLIENT.current_user().await?.model().await?.name;
    ...
    for mapping in role_mappings {
        let new_mapping = DiscordRoleMapping {
            guild_id: mapping.guild_id.to_string().parse().expect("failed to convert snowflake into ID marker"),
            role_id: mapping.role_id.to_string().parse().expect("failed to convert snowflake into ID marker"),
            reward_id: mapping.role_reward.parse().expect("failed to convert role reward into Identifier"),
        };
        by_guild.entry(new_mapping.guild_id).or_default().push(new_mapping);
    }
    ...
    let update_state: Vec<AddUserEntitlementsInput> = db::map_minecraft_users(&mut conn, desired_state)?.iter().map(|(uuid, reward_ids)| AddUserEntitlementsInput {
        uuid: uuid.clone(),
    ...
    for data in update_state {
        client.post(format!("{api_url}/users/add-entitlement", api_url = *API_URL))
            .json(&data)
            .send()
            .await?
            .error_for_status()?;
